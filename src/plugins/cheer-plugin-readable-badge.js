...
import {castArray, compact, isArray, isObject, isString, reduce} from 'lodash'
...
class Badge extends Readable {
  ...
  constructor(badge, options = {}) {
    ...
    if (isString(badge) || isArray(badge)) {
      ...
      const normalizedArgs = isArray(args) ?
        new Map(args) : isObject(args) ?
        reduce(args, (map, v, k) => map.set(k, v), new Map()) : args || new Map()
