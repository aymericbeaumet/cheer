...
import _, {castArray, findIndex, isEmpty, toArray, upperFirst} from 'lodash'
...
class JSDoc extends Readable {
  ...
  filterComments(comments) {
    return isEmpty(this.tags) ? comments :
  ...
  formatTitle(comment) {
    ...
    const args = comment.kind === 'function' ? `(${_(comment.params).map('name').join(', ')})` : ''
    return `${h} \`${path}${args}\``
  }
  ...
  formatArgument(arg, {level = 0} = {}) {
    ...
    const default_ = arg.default ? ' = ' + this.formatDefault(arg.default) : ''
    const description = arg.description ? ' &#x2014; ' + this.formatDescription(arg.description) : ''
  ...
  formatReturn(ret) {
    ...
    const description = ret.description ? ' &#x2014; ' + this.formatDescription(ret.description) : ''
